...
def encrypt_for_master(data):
    # Encrypt the file using PKCS1_OAEP module
    # Only the bot master can read them because only the bot master has the private key
    key = RSA.importKey(open('pastebot.net/master_rsa_public').read())
...
def verify_file(f):
    # Verify if the file was sent by the bot master
    signature = f[:512]
    file = f[512:]
    # Read master_public key
    key = RSA.importKey(open(os.path.join("pastebot.net", "master_rsa_public")).read())
